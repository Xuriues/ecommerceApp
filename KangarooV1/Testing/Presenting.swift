...
class HalfModalPresentationController : UIPresentationController {
    ...
    func changeScale(to state: ModalScaleState) {
        if let presentedView = presentedView, let containerView = self.containerView {
            UIView.animate(withDuration: 0.8, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: .curveEaseInOut, animations: { () -> Void in
                ...
                let halfFrame = CGRect(origin: CGPoint(x: 0, y: containerFrame.height / 3.5),
                                       size: CGSize(width: containerFrame.width, height: containerFrame.height / 1))
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
        return CGRect(x: 0, y: containerView!.bounds.height / 5, width: containerView!.bounds.width, height: containerView!.bounds.height / 1)
    }
